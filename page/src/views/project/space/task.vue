    ...
    export default {
        ...
        methods: {
            ...
            getPreAndNextCode(event) {
                const currentEl = event.item || event.clone;
                const preCode = currentEl ? currentEl.getAttribute('id') : '';
                let toList = [];
                let nextCode = '';
                if (!preCode) {
                    return ['', ''];
                ...
                for (let i = 0, len = event.to.children.length; i < len; i++) {
                    const code = event.to.children[i].getAttribute('id');
                    if (code) {
                        toList.push(code);
                    }
                }
                const preCodeIndex = toList.findIndex(item => item === preCode)
                if (preCodeIndex >= 0 && preCodeIndex < toList.length - 1) {
                    nextCode = toList[preCodeIndex + 1];
                }
                return [preCode, nextCode];
            },
            getTaskStageCode(event) {
                let node = event && event.to ? event.to : null;
                while (node) {
                    if (node.classList && node.classList.contains('scrum-stage-content')) {
            ...
            isTaskSortParamError(msg) {
                if (!msg) {
                    return false;
                }
                return msg.indexOf('参数错误') > -1 || msg.indexOf('参数有误') > -1;
            },
            getLegacyTaskSortPayload(stageCode) {
                const stage = this.taskStages.find(item => item.code === stageCode);
                if (!stage) {
                    return {stageCode, codes: ''};
                }
                const allTasks = [].concat(stage.unDoneTasks || [], stage.doneTasks || []);
                const codes = _.uniq(allTasks.map(item => item && item.code).filter(Boolean)).join(',');
                return {stageCode, codes};
            },
            taskSort(event) {
                const list = this.getPreAndNextCode(event);
                const toStageCode = this.getTaskStageCode(event);
                if (!list[0] || !toStageCode) {
                    this.$message.warning('任务排序失败，请重试', 2);
                    this.getTaskStages(false);
                    return;
                }
                sortTask({preTaskCode: list[0], nextTaskCode: list[1], toStageCode: toStageCode}).then((res) => {
                    if (res && res.code === 200) {
                        return;
                    }
                    if (this.isTaskSortParamError(res && res.msg)) {
                        const legacyPayload = this.getLegacyTaskSortPayload(toStageCode);
                        if (!legacyPayload.codes) {
                            this.getTaskStages(false);
                            return;
                        }
                        sortTask(legacyPayload).then((legacyRes) => {
                            const legacyResult = checkResponse(legacyRes);
                            if (!legacyResult) {
                                this.getTaskStages(false);
                            }
                        }).catch(() => {
