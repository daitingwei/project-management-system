...
## Why
项目中的 `project-user/internal/data` 和 `project-project/internal/data` 文件夹丢失，这些是数据库映射的核心数据模型文件。需要根据 proto 文件、API 模型、DAO 层代码和 domain 层代码反向生成完整的数据模型定义。
...
3. **低优先级**（辅助功能模型）：
   ...
   - task_work_time.go
   - task_log.go
   - task_stages_template.go
   - project_auth_node.go
   - project_log.go
   - file.go
   - source_link.go
