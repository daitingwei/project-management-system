...
func CORS() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		c.Header("Access-Control-Allow-Headers", "Origin, Content-Type, Accept, Authorization, organizationCode")
...
func main() {
	...
	//路由
	min.InitMinio(&min.MinioConfig{
		Endpoint:  config.C.MinioConfig.Endpoint,
		AccessKey: config.C.MinioConfig.AccessKey,
		SecretKey: config.C.MinioConfig.SecretKey,
		Bucket:    config.C.MinioConfig.Bucket,
		UseSSL:    config.C.MinioConfig.UseSSL,
	})
	router.InitRouter(r)
