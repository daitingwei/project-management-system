...
func (p *ProjectAuthNodeDao) FindAllNodes(ctx context.Context) ([]string, error) {
	session := p.conn.Session(ctx)
	var list []string
	err := session.Model(&data.ProjectAuthNode{}).Distinct("node").Select("node").Find(&list).Error
	return list, err
}
