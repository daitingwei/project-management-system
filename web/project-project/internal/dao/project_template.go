...
func (p *ProjectTemplateDao) SaveProjectTemplate(ctx context.Context, pt *data.ProjectTemplate) error {
	return p.conn.Session(ctx).Create(pt).Error
}

func (p *ProjectTemplateDao) UpdateProjectTemplate(ctx context.Context, pt *data.ProjectTemplate) error {
	return p.conn.Session(ctx).Model(pt).Updates(pt).Error
}

func (p *ProjectTemplateDao) Delete(ctx context.Context, code string) error {
	return p.conn.Session(ctx).Where("code=?", code).Delete(&data.ProjectTemplate{}).Error
}

func (p *ProjectTemplateDao) FindProjectTemplateByCode(ctx context.Context, code string) (*data.ProjectTemplate, error) {
	var pt data.ProjectTemplate
	err := p.conn.Session(ctx).Where("code=?", code).First(&pt).Error
	return &pt, err
}
