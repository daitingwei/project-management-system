...
func (t *TaskDao) UpdateTaskDone(ctx context.Context, taskCode int64, done int) error {
	session := t.conn.Session(ctx)
...
// UpdateTask 更新任务
func (t *TaskDao) UpdateTask(ctx context.Context, conn database.DbConn, ts *data.Task) error {
	t.conn = conn.(*gorms.GormConn)
	err := t.conn.Tx(ctx).Where("id=?", ts.Id).Updates(ts).Error
	return err
}
...
func (t *TaskDao) FindTaskMemberByTaskAndMember(ctx context.Context, taskCode int64, memberCode int64) (task *data.TaskMember, err error) {
	err = t.conn.Session(ctx).
...
func (t *TaskDao) UpdateTaskMember(ctx context.Context, tm *data.TaskMember) error {
	return t.conn.Session(ctx).Updates(tm).Error
}
...
func (t *TaskDao) DeleteTaskByStageCode(ctx context.Context, conn database.DbConn, stageCode int) error {
	t.conn = conn.(*gorms.GormConn)
	err := t.conn.Tx(ctx).Where("stage_code=?", stageCode).Delete(&data.Task{}).Error
	return err
}

func (t *TaskDao) RecoverTask(ctx context.Context, taskCode int64, deleted int) error {
	session := t.conn.Session(ctx)
	updates := map[string]interface{}{"deleted": deleted}
	if deleted == 1 {
		updates["deleted_time"] = time.Now().UnixMilli()
	} else {
		updates["deleted_time"] = 0
	}
	err := session.Model(&data.Task{}).Where("id=?", taskCode).Updates(updates).Error
	return err
}

func (t *TaskDao) DeleteTask(ctx context.Context, taskCode int64) error {
	session := t.conn.Session(ctx)
	err := session.Where("id=?", taskCode).Delete(&data.Task{}).Error
	return err
}

func (t *TaskDao) DeleteTaskByPcode(ctx context.Context, pcode int64) error {
	session := t.conn.Session(ctx)
	err := session.Where("pcode=?", pcode).Delete(&data.Task{}).Error
	return err
}

func (t *TaskDao) FindTaskByProjectCodeAndDeleted(ctx context.Context, projectCode int64, deleted int, page int64, size int64) (list []*data.Task, total int64, err error) {
	session := t.conn.Session(ctx)
	offset := (page - 1) * size
	err = session.Model(&data.Task{}).
		Where("project_code=? and deleted=?", projectCode, deleted).
		Limit(int(size)).Offset(int(offset)).
		Find(&list).Error
	if err != nil {
		return
...
func (t *TaskDao) BatchUpdateDeleted(ctx context.Context, taskCodes []int64, deleted int) error {
	session := t.conn.Session(ctx)
	updates := map[string]interface{}{"	deleted": deleted}
	if deleted == 1 {
		updates["deleted_time"] = time.Now().UnixMilli()
	...
	return session.Model(&data.Task{}).Where("id in (?)", taskCodes).Updates(updates).Error
}

func (t *TaskDao) BatchUpdateAssignTo(ctx context.Context, taskCodes []int64, assignTo int64) error {
	session := t.conn.Session(ctx)
	return session.Model(&data.Task{}).Where("id in (?)", taskCodes).Update("assign_to", assignTo).Error
}

func (t *TaskDao) BatchSaveTaskMember(ctx context.Context, members []*data.TaskMember) error {
	session := t.conn.Session(ctx)
	return session.Create(&members).Error
}

func (t *TaskDao) UpdateTaskFields(ctx context.Context, taskCode int64, updates map[string]interface{}) error {
	session := t.conn.Session(ctx)
	return session.Model(&data.Task{}).Where("id=?", taskCode).Updates(updates).Error
}
