...
func (p *ProjectDao) UpdateDeletedProject(ctx context.Context, code int64, deleted bool) error {
	...
	var err error
	if deleted {
		err = session.Model(&data.Project{}).Where("id=?", code).Updates(map[string]interface{}{
			"deleted":      1,
...
func (p ProjectDao) FindProjectByMemId(ctx context.Context, memId int64, condition string, page int64, size int64) ([]*data.ProjectAndMember, int64, error) {
	...
	session := p.conn.Session(ctx)
	index := (page - 1) * size
	sql := fmt.Sprintf("select * from ms_project a, ms_project_member b where a.id = b.project_code and b.member_code=?  %s order by a.sort limit ?,?", condition)
...
func (p *ProjectDao) DeleteProjectMember(ctx context.Context, projectCode int64, memberId int64) error {
	return p.conn.Session(ctx).Where("project_code=? and member_code=?", projectCode, memberId).Delete(&data.ProjectMember{}).Error
}

func (p *ProjectDao) UpdateArchive(ctx context.Context, projectCode int64, archive int) error {
	return p.conn.Session(ctx).Model(&data.Project{}).Where("id=?", projectCode).Update("archive", archive).Error
}

func (p *ProjectDao) DeleteProject(ctx context.Context, projectCode int64) error {
	return p.conn.Session(ctx).Where("id=?", projectCode).Delete(&data.Project{}).Error
}
