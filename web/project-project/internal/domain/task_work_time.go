...
func (d *TaskWorkTimeDomain) DelTaskWorkTime(id int64) *errs.BError {
	c, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
	if err := d.taskWorkTimeRepo.Delete(c, id); err != nil {
		zap.L().Error("DelTaskWorkTime Delete error", zap.Error(err))
