...
func (a *AuthService) SaveAuth(ctx context.Context, msg *auth.AuthReqMessage) (*auth.ApplyResponse, error) {
	organizationCode := encrypts.DecryptNoErr(msg.OrganizationCode)
	pa := &data.ProjectAuth{
	...
	if err := a.projectAuthDomain.SaveAuth(pa); err != nil {
		return nil, errs.GrpcError(err)
	}
	return &auth.ApplyResponse{}, nil
}

func (a *AuthService) UpdateAuth(ctx context.Context, msg *auth.AuthReqMessage) (*auth.ApplyResponse, error) {
	pa := &data.ProjectAuth{
		Id:     msg.AuthId,
		Title:  msg.Title,
		Desc:   msg.Desc,
	...
	if err := a.projectAuthDomain.UpdateAuth(pa); err != nil {
		return nil, errs.GrpcError(err)
	}
	return &auth.ApplyResponse{}, nil
}

func (a *AuthService) SetDefaultAuth(ctx context.Context, msg *auth.AuthReqMessage) (*auth.ApplyResponse, error) {
	organizationCode := encrypts.DecryptNoErr(msg.OrganizationCode)
	if err := a.projectAuthDomain.SetDefault(organizationCode, msg.AuthId); err != nil {
		return nil, errs.GrpcError(err)
	}
	return &auth.ApplyResponse{}, nil
}

func (a *AuthService) DeleteAuth(ctx context.Context, msg *auth.AuthReqMessage) (*auth.DeleteAuthResponse, error) {
	if err := a.projectAuthDomain.DeleteAuth(msg.AuthId); err != nil {
		return nil, errs.GrpcError(err)
	}
	return &auth.DeleteAuthResponse{}, nil
}
